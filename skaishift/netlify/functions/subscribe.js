exports.handler = async (event) => {
  ...
  try {
    ...
    const checkRes = await fetch(
      `https://api.resend.com/audiences/${RESEND_AUDIENCE}/contacts/${encodeURIComponent(email)}`,
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ success: true, already: false }) };
  } catch (err) {
